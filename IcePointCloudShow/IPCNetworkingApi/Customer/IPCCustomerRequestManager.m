...
+ (void)removeOptometry:(NSString *)optometryId CustomerId:(NSString *)customerId SuccessBlock:(void (^)(id))success FailureBlock:(void (^)(NSError *))failure
{
    [self postRequest:@{@"id":optometryId, @"customerId":customerId} RequestMethod:CustomerRequest_RemoveOptometry CacheEnable:IPCRequestCacheDisEnable SuccessBlock:success FailureBlock:failure];
}
