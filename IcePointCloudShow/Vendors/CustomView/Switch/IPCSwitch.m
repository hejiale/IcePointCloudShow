...
- (void)setupUI
{
    self.shape = IPCColorSwitchShapeOval;
    
    [self setBackgroundColor:[UIColor clearColor]];
    
    self.onBackgroundView = [[UIView alloc] initWithFrame:CGRectZero];
    ...
    [self.onBackgroundView mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    self.onBackLabel = [[UILabel alloc] initWithFrame:CGRectZero];
    ...
    [self.onBackLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.onBackgroundView.mas_left).with.offset(15);
        make.right.equalTo(self.onBackgroundView.mas_right).with.offset(0);
        make.top.equalTo(self.onBackgroundView.mas_top).with.offset(0);
        make.bottom.equalTo(self.onBackgroundView.mas_bottom).with.offset(0);
    }];
    
    // Background view for OFF
    self.offBackgroundView = [[UIView alloc] initWithFrame:CGRectZero];
    ...
    [self.offBackgroundView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.mas_left).with.offset(0);
        make.right.equalTo(self.mas_right).with.offset(0);
        make.top.equalTo(self.mas_top).with.offset(0);
        make.bottom.equalTo(self.mas_bottom).with.offset(0);
    ...
    self.offBackLabel = [[UILabel alloc] initWithFrame:CGRectZero];
    self.offBackLabel.font = [UIFont systemFontOfSize:15 weight:UIFontWeightThin];
    self.offBackLabel.textAlignment = NSTextAlignmentRight;
    [self.offBackgroundView addSubview:self.offBackLabel];
    [self.offBackLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.offBackgroundView.mas_left).with.offset(0);
        make.right.equalTo(self.offBackgroundView.mas_right).with.offset(15);
        make.top.equalTo(self.offBackgroundView.mas_top).with.offset(0);
        make.bottom.equalTo(self.offBackgroundView.mas_bottom).with.offset(0);
    }];
...
- (void)setShape:(IPCColorSwitchShape)newShape
{
    if (_shape != newShape)
        _shape = newShape;
    
    if (newShape == IPCColorSwitchShapeOval){
        [self.onBackgroundView.layer setCornerRadius:self.frame.size.height/2];
        [self.offBackgroundView.layer setCornerRadius:self.frame.size.height/2];
        [self.thumbView.layer setCornerRadius:(self.frame.size.height-kHorizontalAdjustment)/2];
    }else if (newShape == IPCColorSwitchShapeRectangle){
    ...
    }else if (newShape == IPCColorSwitchShapeRectangleNoCorner){
